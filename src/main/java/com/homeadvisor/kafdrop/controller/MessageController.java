...
import com.homeadvisor.kafdrop.service.PartitionNotFoundException;
...
public class MessageController {
    ...
    public String viewMessageForm(@PathVariable("name") String topicName,
        ...
        if (messageForm.isEmpty()) {
            ...
            TopicPartitionVO partition = topic.getPartition(0).orElseThrow(() -> new PartitionNotFoundException(topicName));

            defaultForm.setPartition(0);
            if (partition.getSize() >= 100L) {
                defaultForm.setCount(100L);
            } else {
                defaultForm.setCount(partition.getSize());
            }
            defaultForm.setOffset(partition.getSize() - defaultForm.getCount());
            if (defaultForm.getOffset() < 0L) {
                defaultForm.setOffset(0L);
            }

            messageForm = defaultForm;
            isInitialLoad = true;
            model.addAttribute("messageForm", defaultForm);
        }

        if ((!messageForm.isEmpty() && !errors.hasErrors()) || isInitialLoad) {
